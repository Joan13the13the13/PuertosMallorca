...
function updatePorts(ports) {
    ...
    for (let i = 0; i < ports.length; i++) {
        ...
        html += `
          <div class="col-md-3">
            <div class="card">
              <a href="../html/puerto.html?portId=${portId}"><img class="card-img-top" src="` + portImage + `" alt="Card image cap"></a>
              <div class="card-body">
...
function initMap(latit, longi, capa, nomb) {
    ...
    for (let i = 0; i < latit.length; i++) {
        ...
        const infoWindow = new google.maps.InfoWindow({
            content: `<a href="../html/puerto.html?portId=${i}"><strong>${nomb[i]}</strong></a><br>Capacidad: ${capa[i]}`
        });
